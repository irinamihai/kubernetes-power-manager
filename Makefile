...
ifeq (false, $(OCP))
	sed -i 's|^  - \./ocp/rbac\.yaml$$|  - rbac.yaml|' config/rbac/kustomization.yaml
	sed -i 's|^  - \./ocp/role\.yaml$$|  - role.yaml|' config/rbac/kustomization.yaml
else
...
bundle-clean: # Uninstall bundle on cluster using operator sdk.
	$(OPERATOR_SDK) cleanup $(APP_NAME) -n $(KPM_NAMESPACE)
